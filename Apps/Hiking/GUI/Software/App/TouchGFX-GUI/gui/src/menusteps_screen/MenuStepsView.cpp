...
void MenuStepsView::setupScreen()
{
    ...
    menu.setNumberOfItems(Gui::Menu::Settings::Alerts::Steps::ID_COUNT);
    ...
    for (int i = 0; i < Gui::Menu::Settings::Alerts::Steps::ID_COUNT; i++) {
        pS = menu.getSelectedItem(i);

        if (i == 0) {
            Unicode::snprintf(buffer, kBuffSize, "%s", touchgfx::TypedText(T_TEXT_OFF_UC).getText());
        } else {
            Unicode::snprintf(buffer, kBuffSize, "%d", Gui::kStepsList[i]);
        }
...
void MenuStepsView::handleKeyEvent(uint8_t key)
{
    ...
    if (key == Gui::Config::Button::R1) {
        ...
        presenter->saveSteps(Gui::kStepsList[id]);
