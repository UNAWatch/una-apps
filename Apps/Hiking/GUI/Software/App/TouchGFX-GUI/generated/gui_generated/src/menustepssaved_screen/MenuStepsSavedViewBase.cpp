/*********************************************************************************/
/********** THIS FILE IS GENERATED BY TOUCHGFX DESIGNER, DO NOT MODIFY ***********/
/*********************************************************************************/
#include <gui_generated/menustepssaved_screen/MenuStepsSavedViewBase.hpp>
#include <touchgfx/canvas_widget_renderer/CanvasWidgetRenderer.hpp>
#include <touchgfx/Color.hpp>
#include <texts/TextKeysAndLanguages.hpp>
#include <images/BitmapDatabase.hpp>

MenuStepsSavedViewBase::MenuStepsSavedViewBase()
{
    touchgfx::CanvasWidgetRenderer::setupBuffer(canvasBuffer, CANVAS_BUFFER_SIZE);

    __background.setPosition(0, 0, 240, 240);
    __background.setColor(touchgfx::Color::getColorFromRGB(0, 0, 0));
    add(__background);

    title.setXY(50, 0);
    add(title);

    savedText.setXY(73, 150);
    savedText.setColor(touchgfx::Color::getColorFromRGB(192, 192, 192));
    savedText.setLinespacing(0);
    savedText.setWildcard(touchgfx::TypedText(T_TEXT_SAVED).getText());
    savedText.resizeToCurrentText();
    savedText.setTypedText(touchgfx::TypedText(T_TMP_SEMIBOLD_30));
    add(savedText);

    icon.setXY(95, 95);
    icon.setBitmap(touchgfx::Bitmap(BITMAP_MENUASSETS_CIRCLETICK_ID));
    add(icon);

    msg.setPosition(60, 53, 120, 32);
    msg.setColor(touchgfx::Color::getColorFromRGB(192, 192, 192));
    msg.setLinespacing(0);
    Unicode::snprintf(msgBuffer, MSG_SIZE, "%s", touchgfx::TypedText(T_INITIALTEXT).getText());
    msg.setWildcard(msgBuffer);
    msg.setTypedText(touchgfx::TypedText(T_TMP_MEDIUM_25));
    add(msg);
}

MenuStepsSavedViewBase::~MenuStepsSavedViewBase()
{
    touchgfx::CanvasWidgetRenderer::resetBuffer();
}

void MenuStepsSavedViewBase::setupScreen()
{
    title.initialize();
    transitionBegins();
}

void MenuStepsSavedViewBase::transitionBegins()
{
    //Interaction1
    //When screen transition begins change screen to MenuStepsSaved
    //Go to MenuStepsSaved with no screen transition
    application().gotoMenuStepsSavedScreenNoTransition();
}
