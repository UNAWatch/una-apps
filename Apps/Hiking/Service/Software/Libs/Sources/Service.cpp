...
void Service::run()
{
    ...
    mGpsSensor = mKernel.sensorManager.getDefaultSensor(Sensor::Type::SENSOR_TYPE_GPS);
    mHrSensor = mKernel.sensorManager.getDefaultSensor(Sensor::Type::SENSOR_TYPE_HEART_RATE);
    mStepCounterSensor = mKernel.sensorManager.getDefaultSensor(Sensor::Type::SENSOR_TYPE_STEP_COUNTER);
    mFloorCounterSensor = mKernel.sensorManager.getDefaultSensor(Sensor::Type::SENSOR_TYPE_FLOOR_COUNTER);
    mAltimeterSensor = mKernel.sensorManager.getDefaultSensor(Sensor::Type::SENSOR_TYPE_ALTIMETER);
...
void Service::onNewSensorData(const Interface::ISensorDriver* sensor,
    const std::vector<Interface::ISensorData*>& data, bool first)
{
...
void Service::processTrack(std::time_t utc)
{
    ...
    // Creating map
    TrackMapBuilder::GpsPoint newPoint{ mGps.latitude, mGps.longitude};
    // Add point to the track map
    mTrackMapBuilder.addPoint(newPoint);

    // Distance
    float distance = TrackMapBuilder::Distance(newPoint, mPrevGpsPoint);
    ...
    if (trackTimeDiff > 1 && distance > 1.0f) {
        mTrackData.speed = (distance / trackTimeDiff) * 3.6f;
    ...
    if (mTrackData.totalTime > 1 && mTrackData.totalDistance > 1.0f) {
        mTrackData.avgSpeed = (mTrackData.totalDistance / mTrackData.totalTime) * 3.6f;
        mTrackData.avgPace = static_cast<int32_t>(3600.0f / mTrackData.avgSpeed);
    } else {
        mTrackData.avgSpeed = 0.0f;
        mTrackData.avgPace = 0;
    }
    if (mTrackData.lapTime > 1 && mTrackData.lapDistance > 1.0f) {
        mTrackData.avgLapSpeed = (mTrackData.lapDistance / mTrackData.lapTime) * 3.6f;
    ...
    if (lapNum > mTrackData.lapNum) {
        ...
        fitLap.distance = mTrackData.lapDistance;
        fitLap.speedAvg = mTrackData.avgLapSpeed;
...
void Service::stopTrack(bool discard)
{
    ...
    if (!discard) {
        ...
        fitTrack.totalDistance = mTrackData.totalDistance;
        fitTrack.speedAvg = mTrackData.avgSpeed;
