...
Service::Service()
        ...
        , mGlanceTitle()
        , mGlanceValue()
        , mStepsSensor(SDK::Sensor::Type::STEP_COUNTER, this, 1000, 1000)
        , mStepsValue(0)
{
    int16_t w;
    int16_t h;
...
void Service::run()
{
    //mKernel.system.delay(1600);
    mStepsSensor.connect();

    while (!mTerminate) {
        mKernel.system.delay(1000);
    }

    mStepsSensor.disconnect();
}
                           ...
                           bool                                              first)
{
    ...
    if (mStepsSensor.matchesDriver(sensor)) {
        SDK::SensorDataParser::StepCounter sc {sample};
        if (sc.isDataValid()) {
            mStepsValue = sc.getStepCount();
        }
    }
