...
void SideBarSmall::initialize()
{
    SideBarSmallBase::initialize();
}

void SideBarSmall::setCount(uint16_t cnt)
{
    if (cnt == 0) {
        cnt = 1;
    }
...
void SideBarSmall::setActiveId(uint16_t p)
{
    if (mCount <= 1) {
        p = 0;
    }
...
void SideBarSmall::animateToId(int16_t p, int16_t animationSteps)
{
    if (mCount <= 1) {
        p = 0;
    }
...
uint16_t SideBarSmall::getActiveId()
{
    return mPosition;
}


void SideBarSmall::nextAnimationStep()
{
    if (mAnimationRunning) {
        mAnimationCounter++;
...
float SideBarSmall::getStartAngle(uint16_t p)
{
